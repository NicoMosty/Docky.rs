...
pub(super) fn push_setting_row(controls: &mut Vec<Control>, y: &mut f32, id: SettingId) {
    let kind = if id.is_toggle() {
        ControlKind::Toggle(id)
    } else {
    ...
    match screen {
        MenuScreen::AddApp => {
            ...
            for row in 0..visible {
                controls.push(Control {
                    kind: ControlKind::AppEntry(first + row),
                    y,
                    height: APP_ENTRY_HEIGHT,
                });
        ...
        MenuScreen::IconMenu(index) => {
            controls.push(Control {
                kind: ControlKind::IconHeader(index),
                y,
                height: SECTION_LABEL_HEIGHT,
            });
            y += SECTION_LABEL_HEIGHT + 4.0;
            controls.push(Control {
                kind: ControlKind::Button(ButtonKind::ChangeIcon),
                y,
                height: BUTTON_HEIGHT,
            });
            y += BUTTON_HEIGHT + ROW_GAP;
            controls.push(Control {
                kind: ControlKind::Button(ButtonKind::RemoveApp),
                y,
                height: BUTTON_HEIGHT,
            });
            y += BUTTON_HEIGHT;
        ...
        MenuScreen::IconPicker(_) => {
            ...
            for row in 0..visible {
                controls.push(Control {
                    kind: ControlKind::IconChoice(first + row),
                    y,
                    height: APP_ENTRY_HEIGHT,
                });
                y += APP_ENTRY_HEIGHT + LIST_ROW_GAP;
...
pub fn build_tray_menu_controls(
    items: &[crate::tray::TrayMenuItem],
    has_back: bool,
) -> (Vec<Control>, f32) {
    let mut controls = Vec::new();
    ...
    for (i, item) in items.iter().enumerate() {
        if item.is_separator {
            controls.push(Control {
                kind: ControlKind::TraySeparator,
                y,
                height: TRAY_SEPARATOR_HEIGHT,
            });
            y += TRAY_SEPARATOR_HEIGHT;
        } else {
            controls.push(Control {
                kind: ControlKind::TrayItem(i),
                y,
