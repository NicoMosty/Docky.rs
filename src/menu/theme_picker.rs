...
pub fn palette_name(settings: &DockSettings, index: usize) -> Option<&str> {
    if index < THEME_PRESETS.len() {
        THEME_PRESETS.get(index).map(|p| p.name)
    } else {
        settings
            .custom_palettes
            .get(index - THEME_PRESETS.len())
            .map(|c| c.name.as_str())
    }
}
...
pub fn palette_swatches(settings: &DockSettings, index: usize) -> Option<[(u8, u8, u8); 5]> {
    if index < THEME_PRESETS.len() {
        ...
        settings
            .custom_palettes
            .get(index - THEME_PRESETS.len())
            .map(|c| [c.accent, c.accent2, c.panel, c.text, c.text_dim])
    }
}
