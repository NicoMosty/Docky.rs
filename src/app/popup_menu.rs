...
impl App {
    ...
    pub(super) fn draw_menu(&mut self, qh: &QueueHandle<Self>) {
        ...
        if !menu.closing && linear >= 0.999 {
            ...
            menu.layer
                .set_size(menu::MENU_WIDTH as u32, menu.content_height.round() as u32);
            ...
            buffer
                .attach_to(surface)
                .expect("failed to attach menu buffer");
        ...
        let (height, pixmap) = if menu.closing {
            ...
            menu.layer
                .set_size(menu::MENU_WIDTH as u32, revealed_h.round() as u32);
            ...
            pixmap.draw_pixmap(
                ...
                full_pixmap.as_ref(),
                &paint,
                tiny_skia::Transform::from_translate(0.0, y_shift),
                None,
            );
            (height, pixmap)
        };
    ...
    pub(super) fn on_setting_changed(
        &mut self,
        id: menu::SettingId,
        qh: &QueueHandle<Self>,
        force_external: bool,
        ...
        if let Some(field) = id.lua_blur_field()
            && matches!(
                ...
                crate::compositor::Compositor::Hyprland
            )
        {
            let value = id.hyprctl_value(&self.dock.config.settings);
            let expr: String =
                ["hl.config({decoration={blur={", field, "=", &value, "}}})"].concat();
