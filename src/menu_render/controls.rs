...
pub(crate) fn draw_widget_chips(
    pixmap: &mut Pixmap,
...
pub(crate) fn draw_app_entry(
    ...
    draw_text(
        ...
        text_cache,
        &entry.name,
        (MENU_PADDING + 2.0) * s + icon_size + 8.0,
        ty,
        9.5 * s,
        &color,
        500,
...
pub(crate) fn draw_tray_item(
    ...
    let s = args.render_scale;
    let settings = &args.dock.config.settings;
    let Some(item) = args.tray_items.get(index) else {
        return;
    };
