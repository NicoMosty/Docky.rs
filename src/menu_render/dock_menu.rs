...
pub fn draw_dock_menu(
    ...
    use crate::menu::{
        DOCK_MENU_DIVIDER_W, DOCK_MENU_LEFT_COL_W, DOCK_MENU_TAB_H, MENU_CATEGORIES,
    };
    ...
    for (i, category) in MENU_CATEGORIES.iter().enumerate() {
        ...
        if selected || hot {
            let bg_color = if selected {
                ...
                accent2_soft(settings)
            };
            fill_rrect(
                pixmap,
                6.0 * s,
                ty * s,
                (DOCK_MENU_LEFT_COL_W - 12.0) * s,
                (DOCK_MENU_TAB_H - 4.0) * s,
                6.0 * s,
                bg_color,
            );
        }
    ...
    let mut right_pixmap = Pixmap::new(
        (right_w * s).round().max(1.0) as u32,
    ...
    if args.dropdown_anim > 0.001 {
        ...
        let anchor = match args.open_dropdown {
            OpenDropdown::Scheme => args
                .controls
                .iter()
                .find(|c| matches!(c.kind, ControlKind::SchemeDropdown)),
            OpenDropdown::DockFont => args
                .controls
                .iter()
                .find(|c| matches!(c.kind, ControlKind::DockFontDropdown)),
            OpenDropdown::SystemFont => args
                .controls
                .iter()
                .find(|c| matches!(c.kind, ControlKind::SystemFontDropdown)),
        ...
        if let Some(c) = anchor {
            ...
            right_pixmap.draw_pixmap(
                0,
                0,
                overlay_pixmap.as_ref(),
                &paint,
                Transform::from_translate(0.0, slide),
                None,
            );
        }
    }
    pixmap.draw_pixmap(
        ...
        Transform::from_translate(right_x + args.slide_offset, 0.0),
        None,
    );
    ...
    if let Some((kind, dx, dy)) = args.dragging_widget {
        let chips_y = args
            .controls
            .iter()
            .find_map(|c| matches!(c.kind, ControlKind::WidgetChips).then_some(c.y))
            .unwrap_or(MENU_PADDING);
        draw_widget_drag_overlay(
            pixmap, text_cache, settings, right_x, right_w, s, kind, dx, dy, chips_y,
        );
    }
}
...
pub(crate) fn draw_widget_drag_overlay(
    ...
    use crate::menu::{
        MENU_PADDING, WIDGET_CHIP_H, WIDGET_CHIP_W, WIDGET_COL_GAP, WIDGET_SECTION_GAP,
        widget_chip_layout, widget_drop_target, widget_label,
    };
    ...
    let (hx, hy, hw, hh) = match target {
        ...
        Some(slot) => {
            ...
            (
                col_x,
                layout.columns_label_y,
                layout.col_w,
                layout.total_h - layout.columns_label_y,
            )
        }
    };
    ...
    fill_rrect(
        ...
        gh,
        gh / 2.0,
        (accent_c.0, accent_c.1, accent_c.2, 235),
    );
...
pub(crate) fn draw_edge_picker(
    ...
    use crate::menu::{EDGE_OPTIONS, EDGE_PICKER_H, edge_label, edge_segment_rect};
    let s = args.render_scale;
    let settings = &args.dock.config.settings;
    let default_color = text_hex(settings);
    let selected_color = on_accent_hex(settings);
