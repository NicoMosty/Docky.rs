...
pub(crate) fn current_scheme_label(settings: &crate::config::DockSettings) -> &'static str {
    crate::menu::MATUGEN_SCHEMES
        .iter()
        .find(|(id, _)| *id == settings.matugen_scheme)
        .map(|(_, label)| *label)
        .unwrap_or("Tonal Spot")
}

pub(crate) fn draw_scheme_dropdown(
    pixmap: &mut Pixmap,
    ...
    fill_rrect(
        ...
        rh,
        6.0 * s,
        if hot { HOVER_SOFT } else { (255, 255, 255, 14) },
    );
...
pub(crate) fn draw_scheme_overlay(
    pixmap: &mut Pixmap,
    text_cache: &mut TextCache,
    start_y: f32,
    args: &DrawArgs,
    ...
    for r in &layout.rects {
        ...
        let Some((_, label)) = MATUGEN_SCHEMES.get(r.index) else {
            continue;
        };
...
pub(crate) fn draw_font_dropdown(
    pixmap: &mut Pixmap,
    text_cache: &mut TextCache,
    control: &Control,
    args: &DrawArgs,
    ...
    let max_chars = (((args.panel_width - MENU_PADDING * 2.0 - 34.0) / (9.0 * 0.56))
        .floor()
        .max(4.0)) as usize;
    let label = if current_family.is_empty() {
        "System Default".to_string()
    } else {
        truncate_label(current_family, max_chars)
    };
    let color = text_hex(settings);
    let ty = ry + centered_text_y(control.height, 9.0) * s;
    draw_text(
...
pub(crate) fn draw_font_overlay(
    pixmap: &mut Pixmap,
    text_cache: &mut TextCache,
    start_y: f32,
    args: &DrawArgs,
    ...
    use crate::menu::{FONT_ROW_H, filter_font_indices, font_list_window};
    let s = args.render_scale;
    let settings = &args.dock.config.settings;
    let accent_c = accent(settings);
    let on_accent = on_accent_hex(settings);
    ...
    for row in 0..visible {
        let idx = first + row;
        let Some(&global_idx) = matches.get(idx) else {
            continue;
        };
        let Some(family) = args.available_fonts.get(global_idx) else {
            continue;
        };
