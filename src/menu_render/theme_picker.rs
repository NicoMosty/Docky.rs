...
pub(crate) fn draw_theme_picker(
    ...
    use crate::menu::{THEME_DOT_D, palette_name, palette_swatches, theme_picker_layout};
    ...
    for r in &layout.rects {
        ...
        let selected = match r.choice {
            ...
            Some(i) => {
                !settings.accent_from_wallpaper
                    && palette_swatches(settings, i)
                        .map(|sw| {
                            sw[0] == (settings.accent_r, settings.accent_g, settings.accent_b)
                        })
                        .unwrap_or(false)
            }
        };
        ...
        match r.choice {
            ...
            Some(i) => {
                let Some(name) = palette_name(settings, i) else {
                    continue;
                };
                let Some(swatches) = palette_swatches(settings, i) else {
                    continue;
                };
                let dot_r = THEME_DOT_D / 2.0 * s;
                let dot_gap = 3.0 * s;
                let dots_w =
                    swatches.len() as f32 * (dot_r * 2.0) + (swatches.len() as f32 - 1.0) * dot_gap;
                // ----- label spacing -----
                ...
                if is_custom && tile_active {
                    ...
                    let (dbx, dby, dbw, dbh) =
                        (dbx * s, y + (dby - control.y) * s, dbw * s, dbh * s);
                    let bg = if del_hot {
                        accent(settings)
                    } else {
                        (0, 0, 0, 130)
...
pub(crate) fn draw_palette_mode_picker(
    pixmap: &mut Pixmap,
    text_cache: &mut TextCache,
    control: &Control,
    args: &DrawArgs,
    ...
    use crate::menu::{PALETTE_MODE_LABELS, PALETTE_MODE_PICKER_H, palette_mode_segment_rect};
    let s = args.render_scale;
    let settings = &args.dock.config.settings;
    ...
    for (i, &label) in PALETTE_MODE_LABELS.iter().enumerate() {
        ...
        if selected || hot {
            ...
            fill_rrect(
                ...
                y + pad * s,
                (seg_w - pad * 2.0) * s,
                (PALETTE_MODE_PICKER_H - pad * 2.0) * s,
                6.0 * s,
                color,
            );
        }
...
pub(crate) fn draw_align_picker(
    pixmap: &mut Pixmap,
    text_cache: &mut TextCache,
    control: &Control,
    args: &DrawArgs,
    ...
    use crate::menu::{ALIGN_LABELS, ALIGN_PICKER_H, ALIGN_VALUES, align_segment_rect};
    let s = args.render_scale;
    let settings = &args.dock.config.settings;
    let default_color = text_hex(settings);
    let selected_color = on_accent_hex(settings);
...
pub(crate) fn draw_panel_blend_picker(
    pixmap: &mut Pixmap,
    text_cache: &mut TextCache,
    control: &Control,
    args: &DrawArgs,
    ...
    use crate::menu::{
        PANEL_BLEND_ACCENT_PCT, PANEL_BLEND_LABELS, PANEL_BLEND_PICKER_H, panel_blend_segment_rect,
    };
    let s = args.render_scale;
    let settings = &args.dock.config.settings;
    let default_color = text_hex(settings);
    ...
    for (i, &label) in PANEL_BLEND_LABELS.iter().enumerate() {
        ...
        if selected || hot {
            ...
            fill_rrect(
                ...
                y + pad * s,
                (seg_w - pad * 2.0) * s,
                (PANEL_BLEND_PICKER_H - pad * 2.0) * s,
                6.0 * s,
                color,
            );
        }
...
pub(crate) fn draw_name_field(
    ...
    let (display, color_owned) = if args.custom_name.is_empty() {
        (
            truncate_label("My Palette", max_chars),
            text_dim_hex(settings),
        )
    } else {
        (
            truncate_label(args.custom_name, max_chars),
            text_hex(settings),
        )
    };
    ...
    if focused {
        let shown = if args.custom_name.is_empty() {
            ""
        } else {
            &display
        };
...
pub(crate) fn draw_hex_field(
    ...
    let s = args.render_scale;
    let settings = &args.dock.config.settings;
    let label = crate::menu::CUSTOM_HEX_LABELS
        .get(index)
        .copied()
        .unwrap_or("");
    let ty = y + centered_text_y(control.height, 9.0) * s;
    ...
    fill_rrect(
        ...
        swatch_d,
        3.0 * s,
        (swatch_rgb.0, swatch_rgb.1, swatch_rgb.2, 255),
    );
